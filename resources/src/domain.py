...
from enums import CheckForDupesIn, CheckForDupesUsing, EpisodeFilter, FlagMask, JobStatus, JobType, RecordingStatus, ScheduleType
from ffmpeg import FFMPEG
from util import timed, ClientException
...
class CommercialBreak(object):
    ...
    def __repr__(self):
        return "%s {start = %s, end = %s, duration = %s}" % (
            type(self).__name__,                                                  
            util.formatSeconds(self.start),
            util.formatSeconds(self.end),
            util.formatSeconds(self.duration()))
...
class Program(object):
    ...
    def __eq__(self, rhs):
        return isinstance(rhs, type(self)) and \
    ...
    def formattedChannel(self):
        ...
        @rtype: string
        """
        value = self.getChannelNumber()
        if self.getCallSign():
            value += " " + self.getCallSign()
        return value
...
class RecordedProgram(Program):
    ...
    def setBookmark(self, seconds):
        ...
        @type seconds: float
        """
        self.conn().setBookmark(self, util.seconds2frames(seconds, self.getFrameRate()))
    ...
    def getBookmark(self):
        ...
        if not self.isBookmarked():
            ...
            return util.frames2seconds(self.conn().getBookmark(self), self.getFrameRate())
    ...
    def getFileSize(self):
        ...
        @rtype: int
        """    
        return (util.decodeLongLong(int(self._data[10]), int(self._data[9])) / 1024.0)
    ...
    def getFrameRate(self):
        ...
        if not self._fps:
            ffmpegParser = FFMPEG(
                ...
                closeFDs=(type(self._platform) != mythbox.WindowsPlatform),
                windows=(type(self._platform) == mythbox.WindowsPlatform))  
    ...
    def formattedFileSize(self):
        ...
        @rtype: str
        """
        return util.formatSize(self.getFileSize(), True)
...
class Tuner(object):
    ...
    def waitForRecordingToStart(self, timeout, tick=1):
        ...
        if timedOut:
            raise ClientException('Timed out waiting for recording to start on tuner %s' % self)
    ...
    def waitForRecordingWritten(self, numKB, timeout, tick=1, callback=None):
        ...
        while True:
            ...
            time.sleep(tick)
            elapsed += tick
            log.debug('Waiting %s seconds for tuner %s to write %d/%d bytes'%(elapsed, self.tunerId, currPos, numKB))
    ...
    def formattedTunerStatus(self):
        ...
        if tvState.OK == tunerStatus: 
            ...
            status = "Watching %s on %s. Show will end at %s."%(
                recording.title(), 
            ...
            status = "Watched and recording %s on %s. Recording will end at %s"%(
                recording.title(), 
                recording.getChannelName(), 
                time.strftime("%I:%M%p", time.localtime(float(recording.recendtime()))))
        elif tvState.RecordingOnly == tunerStatus: 
            status = "Recording %s on %s. Recording will end at %s"%(
                recording.title(), 
                recording.getChannelName(),
                time.strftime("%I:%M%p", time.localtime(float(recording.recendtime()))))                    
...
class Job(object):
    ...
    def getPercentComplete(self):
        ...
        if self.jobStatus == JobStatus.FINISHED: return 100
            ...
            if m:
                ...
                raise util.StatusException('Percent completion not available: %s' % self.comment)
        else: return 0
    ...
    def getCommFlagRate(self):
        ...
        if m:
            ...
            raise util.StatusException('Comm flag rate not available: %s' % self.comment)
    ...
    def getPositionInQueue(self):
        ...
        if self.jobStatus != JobStatus.QUEUED:
            raise util.StatusException('Job %s with status %s is not in the job queue' % (self.id, self.formattedJobStatus()))
        jobQueue = self.db().getJobs(jobStatus=JobStatus.QUEUED)
        return (jobQueue.index(self) + 1, len(jobQueue))
    
    def moveToFrontOfQueue(self):
        if self.jobStatus != JobStatus.QUEUED:
            raise util.StatusException('Job %s with status %s is not queued' % (self.id, self.formattedJobStatus()))
