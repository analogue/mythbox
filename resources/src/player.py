    ...
    def _buildPlayList(self):
        ...
        if len(comms) > 0:
            ...
            for i, b in enumerate(self._program.getCommercials()):
                if i > 0: title += ', '
                title += util.formatSeconds(b.start)
            title += ')'
...
class Bookmarker(object):
    ...
    def _resumeFromBookmark(self):
        ...
        if bookmarkSecs > 0:
            fb = util.formatSeconds(bookmarkSecs)
            ...
            while self._player.getTime() < bookmarkSecs:
                log.debug('Waiting for player time %s to seek past bookmark of %s' %(util.formatSeconds(self._player.getTime()), fb))
                xbmc.sleep(SLEEP_MILLIS)
        else:
    ...
    def _saveLastPositionAsBookmark(self):
        ...
        log.debug('Setting bookmark on %s to %s' %(self._program.title(), util.formatSeconds(lastPos)))
        try:
            ...
            while self._player.isPlaying():
                ...
                if self._isInBreak(pos):
                    ...
                    if self._isCloseToStartOfCommercial(pos) and not self._wasUserSkippingAround(pos): 
                        ...
                        ui.showPopup(self._program.title(), 'Skipping commercial %s' % util.formatSeconds(self._currentBreak.duration()), 5000)
