...
from util import catchall, catchall_ui, run_async, lirc_hack
...
# =============================================================================
class RecordingDetailsWindow(ui.BaseWindow):
    ...
    def editSchedule(self):
        if self.program.getScheduleId():
            schedule = self.db().getRecordingSchedules(scheduleId=self.program.getScheduleId())[0]
            editScheduleDialog = schedules.ScheduleDialog(
            ...
            xbmcgui.Dialog().ok("Info", "Recording schedule for this program could not be found.")
    ...
    def renderDetail(self):
        ...
        self.setWindowProperty('originalAirDate', s.formattedOrigAirDate())
        ...
        if self.program.isCommFlagged():
            ...
            if len(jobs) == 1:
                ...
                if job.jobStatus == JobStatus.QUEUED:
                    ...
                    try:
                        # TODO: Add ETA 
                        commBreaks = '%d%% at %s fps' % (job.getPercentComplete(), job.getCommFlagRate())
    ...
    def renderThumbnail(self):
        ...
        if thumbFile:
            self.setWindowProperty('thumbnailShadow', 'DialogBack.png')
