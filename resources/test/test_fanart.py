...
class ImdbFanartProviderTest(unittest.TestCase):
    ...
    def test_getRandomPoster_When_program_is_a_movie_Then_returns_fanart(self):
        # Setup
        program = domain.TVProgram({'title':'Fargo', 'category_type':'movie'}, translator=Mock())
        provider = fanart.ImdbFanartProvider(nextProvider=None)
    ...
    def test_getRandomPoster_When_program_is_not_movie_Then_returns_None(self):
        # Setup
        program = domain.TVProgram({'title':'Seinfeld', 'category_type':'series'}, translator=Mock())
        provider = fanart.ImdbFanartProvider(nextProvider=None)
...
class TvdbFanartProviderTest(unittest.TestCase):
    ...
    def test_getRandomPoster_When_program_is_not_movie_Then_returns_poster(self):
        # Setup
        program = domain.TVProgram({'title':'Seinfeld', 'category_type':'series'}, translator=Mock())
        provider = fanart.TvdbFanartProvider(self.platform, nextProvider=None)
    ...
    def test_getRandomPoster_When_program_is_movie_Then_returns_None(self):
        # Setup
        program = domain.TVProgram({'title':'Departed', 'category_type':'movie'}, translator=Mock())
        provider = fanart.TvdbFanartProvider(self.platform, nextProvider=None)
    ...
    def test_getPosters_When_program_the_office_Then_returns_first_result_which_is_wrong_fixme(self):
        # Setup
        program = domain.TVProgram({'title':'The Office', 'category_type':'series'}, translator=Mock())
        provider = fanart.TvdbFanartProvider(self.platform, nextProvider=None)
...
class TheMovieDbFanartProviderTest(unittest.TestCase):
    ...
    def test_getRandomPoster_When_program_is_movie_Then_returns_poster(self):
        # Setup
        program = domain.TVProgram({'title': 'Ghostbusters', 'category_type':'movie'}, translator=Mock())
        provider = fanart.TheMovieDbFanartProvider(nextProvider=None)
...
class GoogleImageSearchProviderTest(unittest.TestCase):
    ...
    def test_getRandomPoster_works(self):
        # Setup
        program = domain.TVProgram({'title': 'Two Fat Ladies', 'category_type':'series'}, translator=Mock())
        provider = fanart.GoogleImageSearchProvider(nextProvider=None)
    ...
    def test_getPosters_works(self):
        # Setup
        program = domain.TVProgram({'title': 'Top Chef', 'category_type':'series'}, translator=Mock())
        provider = fanart.GoogleImageSearchProvider(nextProvider=None)
...
class CachingFanartProviderTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.httpCache = Mock()
        self.program = domain.TVProgram({'title': 'Two Fat Ladies', 'category_type':'series'}, translator=Mock())
        self.provider = fanart.CachingFanartProvider(self.httpCache, self.nextProvider)
        
    def test_getRandomPoster_When_next_link_in_chain_returns_poster_Then_cache_locally_on_filesystem(self):
        # Setup
        when(self.nextProvider).getPosters(any(domain.Program)).thenReturn(['http://www.google.com/intl/en_ALL/images/logo.gif'])
    ...
    def test_getRandomPoster_When_next_link_in_chain_doesnt_find_poster_Then_dont_cache_anything(self):
        # Setup
        when(self.nextProvider).getPosters(any(domain.Program)).thenReturn([])
...
class SuperFastFanartProviderTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        when(self.platform).getScriptDataDir().thenReturn(self.sandbox)
        self.program = domain.TVProgram({'title': 'Two Fat Ladies', 'category_type':'series'}, translator=Mock())
        self.provider = fanart.SuperFastFanartProvider(self.platform, self.nextProvider)
    ...
    def test_getRandomPoster_When_poster_not_in_cache_and_returned_by_next_in_chain_Then_cache_locally_and_return_poster(self):
        # Setup
        when(self.nextProvider).getPosters(any(domain.Program)).thenReturn(['logo.gif'])
...
class OneStrikeAndYoureOutFanartProviderTest(unittest.TestCase):
    ...
    def test_getRandomPoster_When_not_struck_out_and_delegate_returns_none_Then_strike_out_and_return_nextProviders_result(self):
        # Setup
        provider = fanart.OneStrikeAndYoureOutFanartProvider(self.platform, self.delegate, self.nextProvider)
    ...
    def test_getRandomPoster_When_struck_out_Then_skip_delegate_and_return_nextProviders_result(self):
        # Setup
        provider = fanart.OneStrikeAndYoureOutFanartProvider(self.platform, self.delegate, self.nextProvider)
        provider.struckOut['getPosters-%s' % hash(self.program.title())] = self.program.title()
        when(self.nextProvider).getPosters(any()).thenReturn(['blah.png'])
    ...
    def test_constructor_When_delegate_is_none_Then_raise_exception(self):
        try:
            fanart.OneStrikeAndYoureOutFanartProvider(self.platform, delegate=None, nextProvider=self.nextProvider)
