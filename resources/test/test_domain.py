...
import mythtv
import os
import mythbox.platform as platform
import time
import unittest
import util

from mockito import Mock, when, verify, any
from domain import ctime2MythTime, dbTime2MythTime
from domain import Channel, CommercialBreak, Job, TVProgram, Program, RecordedProgram, RecordingSchedule, Tuner
from enums import CheckForDupesIn, CheckForDupesUsing, FlagMask, EpisodeFilter, JobStatus, JobType
...
class RecordedProgramTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.translator = Mock()
        self.platform = Mock()
        self.data = ["0"] * mythprotocol.Protocol40().recordSize()
    ...
    def test_eq_True_same_channelId_and_startttime(self):
        ...
        p1 = RecordedProgram(self.data, self.conn, self.settings, self.translator, self.platform)
        p2 = RecordedProgram(self.data[:], self.conn, self.settings, self.translator, self.platform) 
    ...
    def test_eq_False_different_channelId_and_startttime(self):
        ...
        d2 = ["0"] * mythprotocol.Protocol40().recordSize()
        d2[4] = "101"
        d2[11] = 777777
        p2 = RecordedProgram(d2, self.conn, self.settings, self.translator, self.platform) 
    ...
    def test_formattedAirTime(self):
        ...
        s = RecordedProgram(self.data, self.conn, self.settings, self.translator, self.platform).formattedAirTime()
    ...
    def test_getDuration_When_duration_is_half_hour_Then_return_30mins(self):
        ...
        d = RecordedProgram(self.data, self.conn, self.settings, self.translator, self.platform).getDuration()
    ...
    def test_formattedStartTime_1pm(self):
        ...
        s = RecordedProgram(self.data, self.conn, self.settings, self.translator, self.platform).formattedStartTime()
    ...
    def test_formattedDuration(self):
        ...
        for d in data:
            ...
            self.data[12] = d['end']
            s = RecordedProgram(self.data, self.conn, self.settings, self.translator, self.platform).formattedDuration()
...
class TunerTest(unittest.TestCase):
    ...
    def test_formattedTunerStatus_Success(self):
        # Setup
        from enums import TVState
        self.conn.protocol = mythprotocol.Protocol40()
...
class JobTest(unittest.TestCase):
    ...
    def test_moveToFrontOfQueue_Raises_Exeption_When_Job_Not_Queued(self):
        ...
        except util.StatusException, se:
            log.debug(se)
    ...
    def test_moveToFrontOfQueue_From_10_Of_10(self):
        ...
        when(db).getJobs().thenReturn(jobs)
        import copy
    ...
    def test_getPositionInQueue_Position_Is_1_of_1(self):
        ...
        db = Mock()
        conn = Mock()
        when(db).getJobs().thenReturn([self.createJob(jobStatus=JobStatus.QUEUED, jobType=JobType.COMMFLAG)])
    ...
    def test_isJobFor_ShouldReturnTrue(self):
        ...
        program = RecordedProgram(data=data, conn=Mock(), settings=Mock(), translator=Mock(), platform=Mock())
...
class MythUrlTest(unittest.TestCase):
    ...
    def test_When_url_has_hostname_Then_parse_successful(self):
        url = domain.MythUrl('myth://twiggy:6543/var/lib/recordings/000_111222333444.mpg')
    ...
    def test_When_url_has_ipaddress_Then_parse_successful(self):
        url = domain.MythUrl('myth://192.168.233.212:6543/var/lib/recordings/000_111222333444.mpg')
        self.assertEquals('/var/lib/recordings/000_111222333444.mpg', url.path())
        self.assertEquals('6543', url.port())
